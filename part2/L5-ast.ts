...
import { Result, bind, makeFailure, mapResult, makeOk, safe2, safe3, isFailure, isOk } from "../shared/result";
...
//special kind of bind made for LetValuesExp
export interface LetValuesBind {tag: "LetValuesBind"; variables: VarDecl[]; exp: CExp;}
export const makeLetValuesBind = (varias: VarDecl[], exp:CExp): LetValuesBind => 
    ({tag: "LetValuesBind", variables: varias, exp:exp});
export const isLetValuesBind = (x: any): x is LetValuesBind => x.tag === "LetValuesBind";

//added LetValues
export interface LetValuesExp {tag: "LetValuesExp"; bindings: LetValuesBind[]; body: CExp[]; }
export const makeLetValuesExp = (bindings: LetValuesBind[], body: CExp[]): LetValuesExp => 
...
//bindings as Sexp should be an array, at least 1 set of VarDecls and CExp, otherwise the expression has
//a syntax error
const parseLetValuesExp = (bindings: Sexp, body: Sexp[]): Result<LetValuesExp> =>{
    if(isArray(bindings)){
        const binds: LetValuesBind[]=map(specialValuesParse,bindings); 
        const insides: Result<CExp[]>=mapResult(parseL5CExp, body);
        if(isOk(insides))
            return makeOk(makeLetValuesExp(binds,insides.value));
        else
            return makeFailure(`Invalid body: ${JSON.stringify(body)}`);
    }
    else
        return makeFailure(`Invalid bindings: ${JSON.stringify(bindings)}`);
};

//special function made to parse SExps that represent a LetValues Bind
const specialValuesParse = (arr: Sexp): LetValuesBind =>{
    //
    if((!isArray(arr))||(arr.length!=2)||(!isArray(arr[0]))||(!isArray(arr[1])))
        return makeLetValuesBind([],makeNumExp(5));         //failure
    const vars: Result<VarDecl[]>=mapResult(parseVarDecl,arr[0]);
    const exp: Result<CExp>=parseAppExp(first(arr[1]),rest(arr[1]));
    if(isFailure(vars)||isFailure(exp))
        return makeLetValuesBind([],makeNumExp(5));         //failure
    return makeLetValuesBind(vars.value,exp.value);
};
...
//added values & let-values for Task2
...
export const unparseLetValuesExp = (le: LetValuesExp) : Result<string> => 
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
        (unparseLetValuesBinds(le.bindings), unparseLExps(le.body));

//returns the string with round parentheses 
const addParentheses = (str: string): string => ("(".concat(str)).concat(")");

const unparseLetValuesBinds = (bindings: LetValuesBind[]): Result<string> =>
        makeOk(map(unparseOneBind,bindings).join(" "));

const unparseOneBind = (bind: LetValuesBind): string => {
        const varias: string=addParentheses(map((v: VarDecl): string => `${v.var}` ,bind.variables).join(" "));
        const experssion: Result<string>=unparse(bind.exp);
        if(isOk(experssion))
            return addParentheses(varias.concat(" ",experssion.value));
        else
            return "Failed to parse Let-Values";
}
