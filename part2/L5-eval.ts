...
import { isClosure, makeClosure, Closure, Value, Tuple, isTuple, SExpValue } from "./L5-value";
...
const evalLetValues = (exp: LetValuesExp, env: Env): Result<Value> =>{
    const tuples: Result<Tuple[]> = mapResult((x: LetValuesBind): Result<Tuple> => evalTupleExp(x,env),exp.bindings);
    if(isFailure(tuples))
        return makeFailure("LetValues: Failed to parse CExp or the returning value isn't a Tuple!");
    const vars: VarDecl[]= map((bind: LetValuesBind): VarDecl[] => bind.variables,exp.bindings).reduce((accumulator, value) => accumulator.concat(value), []);
    const values: Value[] = map((tup: Tuple): Value[] => tup.vals,tuples.value).reduce((accumulator, value) => accumulator.concat(value), []);
    if(vars.length != values.length)
        return makeFailure("LetValues: Failure! Tuples members and Variables number aren't equal!");
    return evalSequence(exp.body,makeExtEnv(map((v: VarDecl): string => v.var,vars),values,env));
    //return bind(values, (vals: Value[]) => evalSequence(exp.body, makeExtEnv(vars, vals, env)));
...
const evalTupleExp = (bind: LetValuesBind, env: Env): Result<Tuple> =>{
    const evaluated: Result<Value> = applicativeEval(bind.exp,env);
    if(isOk(evaluated) && isTuple(evaluated.value))
        return makeOk(evaluated.value);
    else
        return makeFailure("Failed to evaluate CExp in body, or it's value isn't a Tuple!");
}
