...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, CExp, isLetValuesExp, LetValuesExp, LetValuesBind } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, makeNonEmptyTupleTExp, isTupleTExp, makeEmptyTupleTExp, littleParseTExp, isNonTupleTExp } from "./TExp";
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>
    bind(typeofExp(app.rator, tenv), (ratorTE: TExp) => {
        if(isTupleTExp(ratorTE)){
            if(app.rands.length==0)
                return makeOk(makeEmptyTupleTExp());
            const operands: Result<TExp[]> = mapResult((exp: CExp) => typeofExp(exp,tenv),app.rands);
            if(isOk(operands) && allT(isNonTupleTExp,operands.value))
                return makeOk(makeNonEmptyTupleTExp(operands.value));
        }
...
//no need to check CExps types, L5-Eval will counter any problems we could have.
export const typeofLetValues = (exp: LetValuesExp, tenv: TEnv): Result<TExp> =>{
    const tmp= map((bind: LetValuesBind) => bind.variables,exp.bindings).reduce((accumulator, value) => accumulator.concat(value), []);
    const vars = map((v) => v.var, tmp);
    const varTEs = map((v) => v.texp, tmp);
